...
function shell(content,active,identidade){return `<div class="app-shell-dashboard" data-dashboard-surface="${active}"><aside class="dashboard-sidebar"><div class="dashboard-brand"><img class="brand-logomark" src="./icon.svg" alt=""><div><strong><span class="wordmark-med">Med</span><span class="wordmark-per">Per</span></strong><span>Perícia estruturada</span></div></div>${nav(active)}<div class="dashboard-profile"><span class="profile-avatar">${esc(identidade.iniciais)}</span><div><strong>${esc(identidade.rotulo)}</strong>${identidade.nome&&identidade.email?`<span>${esc(identidade.email)}</span>`:''}</div></div></aside><main id="workspace" class="dashboard-workspace">${content}</main></div>`;}
...
// Identidade de quem está usando. Antes disto a barra lateral trazia as iniciais
// "JR", o papel "Perita médica" e o nome "Dra. Joyce" escritos no código: num
// piloto com várias peritas, todas veriam a identidade da mesma pessoa. Agora
// tudo deriva da sessão, e a ausência de dado não é preenchida com invenção.
function accountIdentity({displayName='',accountEmail=''}={}){
  ...
  const iniciais=partes.length
    ...
    : (email ? email.slice(0,2).toUpperCase() : 'MP');
  return {nome,email,iniciais,rotulo:nome||email||'Sua conta'};
}
...
function overview(state,filter,options){
  ...
  return `${pageHeader(options.displayName?`Boa noite, ${options.displayName}`:'Boa noite','Priorize o trabalho sem perder o contexto médico-pericial.')}<div class="work-layout">${work}${deadlineBlock}${pendingBlock}${listaBlock}${quiet}</div>`;
}
...
export function renderDashboardSurface(state,surface='overview',filter='active',options={}){const safe=SURFACES.has(surface)?surface:'overview';const identidade=accountIdentity(options);const displayName=identidade.nome;const content=safe==='cases'?casesSurface(state,filter,options):safe==='deadlines'?deadlinesSurface(state,filter,options):safe==='indicators'?indicatorsSurface(state,options):safe==='references'?referencesSurface():safe==='models'?modelsSurface(state,options):overview(state,filter,{...options,displayName});return shell(content,safe,identidade);}
