...
// Pergunta opcional precisa de caminho de volta. Radio não desmarca por clique:
// sem um controle explícito, um clique acidental grava um valor e a perita fica
// sem como restaurar o estado "não declarado" — que é legítimo justamente porque
// o campo é opcional. O controle só aparece quando há algo a limpar.
function choices(path,label,value,options,optional=false){const legacy=value&&!options.some(o=>optionId(o)===value);const list=legacy?[...options,value]:options;return`<fieldset class="guided-question"><legend>${esc(label)}</legend><div class="guided-choices">${list.map(o=>{const id=optionId(o),isLegacy=legacy&&id===value;return`<label class="guided-choice${isLegacy?' is-legacy':''}"><input type="radio" name="${esc(path)}" data-bind="${esc(path)}" value="${esc(id)}" ${value===id?'checked':''}><span>${esc(optionLabel(o))}${isLegacy?'<small>registro anterior — fora da escala atual</small>':''}</span></label>`;}).join('')}</div>${optional&&value?`<button type="button" class="guided-clear" data-clear-choice="${esc(path)}">Limpar resposta</button>`:''}</fieldset>`;}
...
// que a auditoria existe para fazer. Nenhuma regra de engine.js foi alterada.
//
// O painel precisa existir em toda etapa que seja destino de roteamento: dizer que
// uma pendência pertence a outra etapa e não mostrá-la ao chegar lá é pior do que
// não rotear. A Conclusão é a exceção deliberada — lá o controle de suficiência
// exibe a lista inteira, sem escopo, porque é o momento em que ela decide.
function stageAudit(c,stageId,extraCounts=''){
  const a=auditCase(c);
  const here=a.issues.filter(i=>stageForAuditField(i.field)===stageId);
  const elsewhere=a.issues.filter(i=>stageForAuditField(i.field)!==stageId);
  const totals=`<p class="audit-totals">${plural(a.blocks,'bloqueio','bloqueios')} e ${plural(a.warnings,'ressalva','ressalvas')} no caso${extraCounts}</p>`;
