...
const fakeFetch=async(url,options={})=>{
  ...
  if(String(url).endsWith('/cases')&&(!options.method||options.method==='GET')){
    return {ok:true,status:200,json:async()=>[{id:'remote-1',title:'Caso',reference:'0001',objectType:'Dano corporal'}]};
  }
...
const client=createCaseStateClient({baseUrl:'https://api.medper.test',getAccessToken:()=> 'token-123',fetchImpl:fakeFetch});
const listed=await client.listCases();
assert.equal(listed[0].id,'remote-1');
...
assert.equal(calls[2].options.headers.Authorization,'Bearer token-123');
